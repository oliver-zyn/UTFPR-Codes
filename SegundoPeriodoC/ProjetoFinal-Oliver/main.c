...
void atualizaAluno(Pilha* pAlunos, Pilha* pUsuarios, int RA, char nome[], char cpf[], char email[], char senha[]) {
...
void atualizaProfessor(Pilha* pProfessores, Pilha* pUsuarios, int codProfessor, char nome[], char cpf[], char email[], char senha[]) {
...
void loginAdm(Pilha *pAlunos, Pilha *pProfessores, Pilha *pUsuarios) {
  ...
  do {
    ...
    case 3:
      ...
      gets(senha);
      fflush(stdin);
      atualizaAluno(pAlunos, pUsuarios, RA, nome, cpf, email, senha);
...
int solicitaLoginProfessor(Pilha *pUsuarios) {
  ...
  for (int i = pUsuarios->topo; i >= 0; i--) {
    ...
    if (usuarioAtual->email == email && usuarioAtual->senha == senha && usuarioAtual->professor == 1) {
...
void loginProfessores(Pilha *pAlunos, Pilha *pProfessores, Pilha *pUsuarios, Pilha *pTurmas) {
  ...
  int codUsuarioProfessorLogado = 0, codProfessorLogado = 0, logou = 0, opcao = 0, qtdMaxAlunos = 0;
